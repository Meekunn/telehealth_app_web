			...
			lessonKey,
			{
				...
				subHeadings: Object.fromEntries(
					Object.entries(lessonValue.subHeadings).map(([key, value]) => [
						...
						{
							...value,
							content: React.createElement(React.Fragment, null, parse(value.content)),
						},
					])
				),
			},
		...
		<Routes>
			<Route path={mainPaths.BASE} element={<Dashboard />}>
				...
				<Route path={mainPaths.MODULE} element={<MainPage />} key="mainPage">
					<Route index element={<Navigate to={`${mainPaths.MODULE}/${Object.keys(lessonData)[0]}`} replace />} />
				...
				<Route path={mainPaths.MODULE} element={<Sidebar onClose={onClose} />} />
