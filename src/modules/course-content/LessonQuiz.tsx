...
function LessonQuiz({ quizzes }: LessonQuizProps) {
								...
								return (
									...
									>
										{currentQuiz.type === 'multiple-choice' ? (
											...
											<Checkbox colorScheme="brand,white" isChecked={isChecked} onChange={() => handleOptionChange(quizKey, key)}>
												{option.text}
											</Checkbox>
										)}
									</Box>
				...
				return (
					...
					>
						<Select
							...
							onChange={(e) => setAnswers({ ...answers, [quizKey]: e.target.value })}
							variant="unstyled"
							sx={{
								'& option:first-child': {
									marginTop: '16px',
	...
	const calculateScore = () => {
		...
		Object.keys(quizzes).forEach((quizKey) => {
			...
			// Check if correctAnswers exist
			const correctAnswers = currentQuiz?.correctAnswers || [];

			if (currentQuiz.type === 'multiple-choice' || currentQuiz.type === 'dropdown') {
				// For single-answer questions, compare the first element of correctAnswers and userAnswers
				if (userAnswers && correctAnswers[0] && userAnswers[0] === correctAnswers[0]) {
					calculatedScore++;
				}
			} else if (currentQuiz.type === 'select-multiple') {
				// For multiple correct answers, ensure both arrays match after sorting
				if (
				...
				// For text answers, compare the user's answer with the correct answer (case insensitive)
				if (
					typeof userAnswers === 'string' &&
					typeof correctAnswers[0] === 'string' &&
					userAnswers.toLowerCase() === correctAnswers[0].toLowerCase()
				) {
					calculatedScore++;
				}
			}
		...
		<Box width="100%" height="100%">
			{Object.keys(quizzes).map((quizKey, index) => {
				...
				return (
					<Box key={quizKey} py="70px" px="16px" bgColor="brand.white">
						<Text as="h3" pt="20px" pb="10px" lineHeight="40px" fontSize="xx-large" fontWeight="bold">{`Question ${
							index + 1
						}`}</Text>
						{/* <Text py="20px" mb={4}>
			...
			<Box py="64px" px="8px" bgColor="#f0f0f0">
				...
				<Box w="80%" h="100%" m="auto">
					<Button w="100%" h="100%" py={5} color="brand.white" bg="green.400" onClick={handleSubmit} _hover={{ bg: 'green.500' }}>
						Submit
					</Button>
				</Box>
			</Box>
